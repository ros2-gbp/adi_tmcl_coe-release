...
/* Constructor */
TmcCoeStepperMotor::TmcCoeStepperMotor(ros::NodeHandle *p_nh, TmcCoeInterpreter* p_tmc_coe_interpreter,
                         uint8_t slave_number, uint8_t motor_number) :
TmcCoeMotor(p_nh, p_tmc_coe_interpreter, slave_number, motor_number)
{
  ROS_DEBUG_STREAM("[TmcCoeStepperMotor::" << __func__ << "] called");
}
...
void TmcCoeStepperMotor::init()
{
  ...
  ROS_INFO_STREAM("[TmcCoeStepperMotor::" << __func__ << "] called");
  ...
  if(position_scaler_ == 0 && commutation_mode_ > STEPPER_OPENLOOP_MOTOR)
  {
...
void TmcCoeStepperMotor::initSubscriber()
{
  ...
  {
    case STEPPER_DISABLED_MOTOR:
      ROS_INFO_STREAM("[" << __func__ << "] Commutation Mode : DISABLED");
      break;

    case STEPPER_OPENLOOP_MOTOR:
